<script>
  ...
  onMount(async () => {
    try {
      ...
      const productsRes = await fetch("https://192.241.150.36.nip.io/api/products");
      ...
      const locationsRes = await fetch("https://192.241.150.36.nip.io/api/locations");
  ...
  async function search() {
    ...
    materialRiskScores = {};
    try {
      const res = await fetch("https://192.241.150.36.nip.io/api/analyze", {
        method: "POST",
        ...
        for (const material of result.raw_materials) {
          try {
            if (result.material_source_locations[material] && result.material_source_locations[material].length > 0) {
              ...
              const riskRes = await fetch(`https://192.241.150.36.nip.io/api/risk_score?${locations.map(loc => `locations=${encodeURIComponent(loc)}`).join('&')}&material=${encodeURIComponent(material)}`);
              if (riskRes.ok) {
